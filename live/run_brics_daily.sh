#!/bin/bash
# run_brics_daily.sh — brics Gulf EM diversifier (Blaque Baux). DRY-RUN by default (logs the target,
# places nothing) using the shared read-only data keys; graduates to PAPER once
# ~/.config/blaquebaux/alpaca_brics.env exists (that account's own keys). One-time:
# julia --project=engine -e 'using Pkg; Pkg.instantiate()'.  Manual dry test: BB_DRYRUN=1 bash live/run_brics_daily.sh
set -uo pipefail
REPO="/Users/malcolmx/blaquebaux-brics"; ENGINE="$REPO/engine"; JULIA="/Users/malcolmx/.juliaup/bin/julia"
DATAENV="$HOME/.config/blaquebaux/alpaca.env"; SLEEVEENV="$HOME/.config/blaquebaux/alpaca_brics.env"
LOGDIR="$REPO/logs"; mkdir -p "$LOGDIR"; LOG="$LOGDIR/brics_$(TZ=America/New_York date +%Y%m%d).log"
exec >> "$LOG" 2>&1
echo "======== $(TZ=America/New_York date '+%F %T %Z') brics daily run ========"
export BB_LEDGER_PATH="$REPO/alpaca_ledger_brics.sqlite" BB_AUDIT_PATH="$REPO/alpaca_audit_brics.jsonl"
export BB_HWM_PATH="$HOME/.config/blaquebaux/equity_hwm_brics.txt" BB_EQUITY_PATH="$HOME/.config/blaquebaux/equity_last_brics.txt"
...
cd "$REPO" || exit 1
"$JULIA" --project="$ENGINE" "$REPO/live/brics_live.jl"; RC=$?
